...
class HyperlinkedImage(Image, object):
    ...
    def drawOn(self, canvas, x, y, _sW=0):
        if self.hyperlink:  # If a hyperlink is given, create a canvas.linkURL()
            x1 = x
            y1 = y
            x2 = x1 + self._width  # type: ignore
            y2 = y1 + self._height  # type: ignore
            canvas.linkURL(
                url=self.hyperlink, rect=(x1, y1, x2, y2), thickness=0, relative=1
            )
        super(HyperlinkedImage, self).drawOn(canvas, x, y, _sW)  # type: ignore
...
def list_works(orcid_dir):
    ...
    for i, w in enumerate(work_xml_list):
        ...
        print(
            str(i)
            + ": "
            + xml_dict["work:work"]["work:title"]["common:title"]
            + " ("
            + xml_dict["work:work"]["@put-code"]
            + ")"
        )
...
def load_affiliation(affiliation_path):
    ...
    if affiliation_dict["end_date"] == "":
        affiliation_dict["date_range"] = affiliation_dict["start_date"] + " - present"  # type: ignore
    else:
        affiliation_dict["date_range"] = (
            affiliation_dict["start_date"] + " - " + affiliation_dict["end_date"]  # type: ignore
        )  # type: ignore
...
def load_work(work_path):
    ...
    out_work_dict = {
        "type": in_work_dict[
            "work:type"
        ],  # This field is used for sorting entries and defines other behavior
    ...
    if isinstance(external_ids, list):
        out_work_dict["external_ids"] = [
            ...
            if d["common:external-id-type"] == "doi"
        ]
    ...
    if not out_work_dict["authors"] == "":
        if isinstance(out_work_dict["authors"], dict):
            ...
            out_work_dict["authors"] = [
                i["work:credit-name"] for i in out_work_dict["authors"]
            ]
...
def prune_duplicate_works(work_dict):
    ...
    while check_duplicates(work_dict):
        ...
        if del_key:
            print(
                f"Merging {work_dict[del_key]['title']} into {work_dict[keep_key]['title']}"
            )
            work_dict[keep_key]["external_ids"] += work_dict[del_key]["external_ids"]
            work_dict[keep_key]["external_ids"] = list(
                set(work_dict[keep_key]["external_ids"])
            )
            ...
            if "eLife" in w["doi"]:
                ...
                try:
                    ...
                    if "www." in url:
                        ...
                        slash_idx1 = url.find("//") + 2
                        slash_idx2 = url.find("/", slash_idx1)
                        url = url[slash_idx1:slash_idx2]
...
def load_review(review_path):
    ...
    r = requests.get(f"https://portal.issn.org/resource/ISSN/{str(issn)}")
    potential_name = ""
    for l in r.iter_lines():
        line = str(l)
        if issn in line:
            potential_name = line[28:-9]
            if "(" in potential_name:
                idx = potential_name.find("(")
                potential_name = potential_name[: idx - 1]
            break
    if potential_name == "":
...
def extract_orcid_info(orcid_dir):
    ...
    personal = {
        ...
        "links": {
            ...
            + get_recursive_key(personal_info, "person:name", "@path")  # type: ignore
        },
    }
    ...
    if "researcher-url:researcher-urls" in personal_info.keys():
        ...
        for i in range(len(link_list)):
            personal["links"][link_list[i]["researcher-url:url-name"]] = link_list[i][  # type: ignore
                "researcher-url:url"  # type: ignore
            ]
    # Get primary email
    personal["email"] = [
        e["email:email"]
        for e in personal_info["email:emails"]["email:email"]
        if e["@primary"] == "true"
    ][0]
...
def folder_to_dict(path, load_fun) -> dict:
    _dict = {}
...
def make_affiliation_table(config, affiliation, section_heading=""):
    table_data, table_style = None, None
    if config["style"] == "greenspon-default":
        if section_heading == "":
            table_data = [
                [
                    ...
                    Paragraph(
                        affiliation["date_range"], style=config["item_date_style"]
                    ),
                ],
                [
                    Paragraph(
        ...
        if section_heading == "":
            table_data = [
                [
                    ...
                    Paragraph(work_date, style=config["item_date_style"]),
                ],
                [work_body, ""],
            ]
            table_style = [
...
def add_affiliation_section(
    elements, orcid_dict: dict, config, heading, affiliation_type
):
    # Compute column size
    ...
    affiliations = sorted(
        affiliations, key=lambda v: int(v["start_date"]), reverse=True
    )
    ...
    for af in affiliations:
        ...
        # Convert to table
        t = Table(table_data, colWidths=column_widths)  # type: ignore
        t.setStyle(table_style)  # type: ignore
        # Append
        elements.append(t)
        elements.append(Spacer(0, config["item_spacing"]))
...
def add_work_section(elements, orcid_dict, config, heading, search_str):
    ...
    works = sorted(
        works, key=lambda v: int(v["year"]) * 1000 + int(v["month"]), reverse=True
    ...
    for work in works:
        ...
        work_body = Paragraph(
            work_str + "<br/>" + author_cat, style=config["item_body_style"]
        )
        # prepare table
        if is_heading:
...
class FooterCanvas(canvas.Canvas):
    ...
    def draw_canvas(self, page_count):
        page = "Page %s of %s" % (self._pageNumber, page_count)  # type: ignore
        y = 40
        self.saveState()
...
def make_document_config(style) -> dict:
    config = {}
    if style.lower() == "greenspon-default":
        config = {
            ...
            "item_body_style": ParagraphStyle(
                ...
                fontName="Helvetica",
                underlineWidth=1,
                underlineOffset="-0.1*F",  # type: ignore
            ),
        }
    else:
        ValueError("Invalid style")
